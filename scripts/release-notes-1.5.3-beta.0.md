...
- Skips: broadcaster (`S.twLogin`), `*bot$` patterns, configurable exclude list (defaults: `nightbot streamlabs streamelements moobot fossabot soundalerts tangiabot`).
...
- Per-stream reset hooked into `startStreamSession` so a regular gets re-evaluated next stream.
...
- Runtime state (`currentMessageId`, `currentWebhookUrl`, `liveStartedAt`) deliberately not persisted. SF restart mid-stream means the next end-event silently skips the recall instead of trying to PATCH a stale id.
- Test Live-Post button verifies template + role-ping setup without waiting for a real go-live signal.

### E. Chat overlay (`obs-overlays/chat.html`)

- `theme-box`: linear-gradient overlay (brand blue/teal twist over the existing translucent dark base) replaces the flat translucent rectangle. Soft drop-shadow, slightly thicker padding.
...
- `applyConfig` treats `theme` as a 3-way enum (`card | box | nobg`) with graceful fallback to `card` for unknown values from legacy configs.
...
- SF renderer subscribes to `/api/integrations/events` and renders product cards with status dot, version chip, capability tags, and optional now-playing meta.
...
- Full protocol contract in `INTEGRATIONS.md` at the repo root — endpoint shapes, capability vocabulary, HTTPS→loopback mixed-content gotcha, additive-only versioning policy.

### G. Verification

- JS parses cleanly (505k chars).
- `node scripts/test-auth-suite.js` — 25/25 pass.
- `node scripts/test-patreon-entitlement.js` — 16/16 pass.
- SBAE end-to-end round-trip: 5 refs Find Targets, 2 refs Start Raid, UUIDs match the JS constants.
- `obs-server` module loads and exposes `listIntegrations`.

### Known follow-ups (still pending from earlier)
...
- `StreamFusion.html` landing-page screenshot grid (sitting in working tree; needs `marketing/` committed first).
- `rotating-bar.html` (untracked, unknown provenance — needs triage).
